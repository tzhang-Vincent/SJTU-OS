...
FORWARD _PROTOTYPE( struct proc * pick_proc_lott, (void));
FORWARD _PROTOTYPE( struct proc * pick_proc_edf, (void));
  ...
  if (! time_left) {				/* quantum consumed ? */
      ...
      if (rp->p_priority < (NR_SCHED_QUEUES-1)) {
          rp->p_priority += 1;			/* lower priority */
      	  rp->p_lottery = 10*(NR_SCHED_QUEUES - rp->p_priority);
      }
  }
 ...
 *				pick_proc_edf				     * 
 *===========================================================================*/
PRIVATE struct proc * pick_proc_edf(void)
{
/* Decide who to run now.  A new process is selected an returned.
 * When a billable process is selected, record it in 'bill_ptr', so that the 
 * clock task can tell who to bill for system time.
 */
  register struct proc *rp;			/* process to run */
  int q;				/* iterate over queues */
  clock_t s_exp = 0;    /* smallest expiration time */
  register struct proc *s_rp = NULL; /* smallest expiration time process*/
  /* Check each of the scheduling queues for ready processes. The number of
   * queues is defined in proc.h, and priorities are set in the task table.
   * The lowest queue contains IDLE, which is always ready.
   */
  ...
  for (q=0; q < NR_SCHED_QUEUES; q++) {	
	...
	while(rp){
		...
		if(s_exp == 0 && s_rp == NULL){
			s_exp = rp->p_deadline.tmr_exp_time;
			s_rp = rp;
			rp = rp->p_nextready;
			continue;
		}
		s_exp = (rp->p_deadline.tmr_exp_time < s_exp) ? rp->p_deadline.tmr_exp_time:s_exp;
		s_rp = (rp->p_deadline.tmr_exp_time < s_exp) ? rp:s_rp;
		rp = rp->p_nextready;
	}
	}
  }
 ...
 *				pick_proc_lott				     * 
 *===========================================================================*/
PRIVATE struct proc * pick_proc_lott(void)
{
/* Decide who to run now.  A new process is selected an returned.
 * When a billable process is selected, record it in 'bill_ptr', so that the 
 * clock task can tell who to bill for system time.
 */
  ...
  int count = 0;
  if(total>0){
  	selected = rand() % total + 1;/*select the ticket in the lottery pool*/
  }
  for (q=7; q < NR_SCHED_QUEUES; q++) {	
   	if(!(rp = rdy_head[q])) {
		TRACE(VF_PICKPROC, printf("queue %d empty\n", q););
  ...
  for (rp=BEG_PROC_ADDR; rp<END_PROC_ADDR; rp++) {
	  ...
	  if (rp->p_priority > rp->p_max_priority) {	/* update priority? */
	      ...
	      rp->p_priority -= 1;			/* raise priority */
  		  rp->p_lottery = 10*(NR_SCHED_QUEUES - rp->p_priority); /*update tickets*/
